...
const char *PROGNAME = "Atividade 8.2 – Cubo tonalizacao gouraud, iluminacao phong com textura";
const char *BASEDIR = "/home/vini-carvalho/Documentos/EA979/OpenGL (Cópia 2)/Atividade8/Atividade_8_2";
...
GLuint VAO[1];
GLuint VBO[4]; //vertices, normais, cores, textura
...
GLuint textureID; // Variável para guardar o ID da textura
...
void display(GLFWwindow* window) {
   ...
   // 	fflush(stdout);
        ...
        // Posição da Câmera/Observador. Com viewMatrix identidade, a câmera está em (0,0,0)
        glUniform3f(viewPosLoc, 0.0f, 5.0f, 5.0f);
        // Cor da luz 
        glUniform3f(lightColorLoc, 1.0f, 1.0f, 1.0f);
        // Posição da Luz
        GLfloat lightPos[] = {0.0f, 5.0f, 5.0f};
...
void init(void) {
    ...
    // ---------------------------------------
    glGenBuffers(4, VBO); //gera os 4 buffers: vertices, normals, colors, textura
    ...
    glBindTexture(GL_TEXTURE_2D, textureID); // Ativa a textura
    ...
    //inverte a imagem (as vezes precisa)
    stbi_set_flip_vertically_on_load(true);
    unsigned char *tex_data = stbi_load("box (1).jpg", &width, &height, &channels, 0);

    if (tex_data) {
        // Envia os dados da imagem para o objeto de textura ativo
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO[3]); // Use VBO[3], o quarto buffer
    glBufferData(GL_ARRAY_BUFFER, planeNumVertices * 2 * sizeof(GLfloat), texels, GL_STATIC_DRAW);
    glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 0, (GLvoid*)0);
    glEnableVertexAttribArray(3); // Habilita o location = 3
