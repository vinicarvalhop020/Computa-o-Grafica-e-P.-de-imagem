...
GLfloat* tonalizacao_gouraud(GLfloat *vertices, int numVertices, int resolucao){
...
GLint createPlane(GLfloat **vertex, GLboolean colorFlag, GLfloat **color,  GLboolean normalFlag, GLfloat **normal, GLboolean texelFlag, GLfloat **texel) {
  ...
  *vertex = esfera_triangulos(&numVertices, resolucao); 

  //gouraud
  //*normal = tonalizacao_gouraud(*vertex, numVertices, *vertex); //normal gouraud
  //flat
...
//  fprintf(stdout, "plane %f", *plane[0]);
    ...
    if (texelFlag == GL_TRUE) {
        *texel = NULL; // Não estamos usando texturas neste exemplo
    }
